...
{
    public class CameraConfidenceHandler : CallbackHandler
    {
        private readonly IApiClient _apiClient;

        public CameraConfidenceHandler(IApiClient apiClient) : base(apiClient)
        {
        ...
        protected override async Task HandleCallbackAsync(ITelegramBotClient botClient, Update update, CancellationToken cancellationToken)
        {
            ...
            if ( parts.Length > 2 && Int32.TryParse(parts[1], out var conf) && Int32.TryParse(parts[2], out var cameraId) )
            {
